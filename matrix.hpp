...
template<typename ValueT>
Matrix<ValueT> Matrix<ValueT>::extra_borders(int kernel_vert_radius, int kernel_hor_radius) const
{
	...
	//top and bottom
	for (int i = 0; i < kernel_vert_radius; i++) {
		for (int j = 0; j < n_cols; j++) {
	...
	//left and right
	for (int i = 0; i < n_rows; i++) {
		for (int j = 0; j < kernel_hor_radius; j++) {
